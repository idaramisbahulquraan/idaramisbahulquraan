...
const ManageIncome = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      const cSnap = await getDocs(query(collection(db, 'incomeCategories')));
      const tSnap = await getDocs(query(collection(db, 'incomeTransactions')));
  ...
  const addCategory = async (e) => {
    e.preventDefault();
    setSavingCat(true);
    const docRef = doc(collection(db, 'incomeCategories'));
    await setDoc(docRef, {
      uid: docRef.id,
      name: catForm.name,
  ...
  const deleteCategory = async (id) => {
    await deleteDoc(doc(db, 'incomeCategories', id));
  ...
  const addTransaction = async (e) => {
    e.preventDefault();
    setSavingTx(true);
    const docRef = doc(collection(db, 'incomeTransactions'));
    await setDoc(docRef, {
      uid: docRef.id,
  ...
  const deleteTransaction = async (id) => {
    await deleteDoc(doc(db, 'incomeTransactions', id));
    ...
    <div className="max-w-6xl mx-auto p-8 space-y-8">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-gray-500">Manage Income</div>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <form onSubmit={addCategory} className="bg-white shadow-md rounded px-8 pt-6 pb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <button type="submit" disabled={savingCat} className={`px-4 py-2 bg-green-600 text-white rounded ${savingCat ? 'opacity-50 cursor-not-allowed': ''}`}>{savingCat ? 'Saving...' : 'Add'}</button>
        </div>
      </form>
