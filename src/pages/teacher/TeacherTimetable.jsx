...
const TeacherTimetable = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const q = query(collection(db, 'timetables'), where('teacherId', '==', currentUser.uid));
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-gray-500">My Timetable</div>
          </div>
        </div>
        <div className="flex items-center gap-3">
              ...
              {byDay[day]
                ...
                .map((e, idx) => (
                  <li key={idx} className="px-6 py-3 flex justify-between">
                    <span>{e.subject} (Class {e.className})</span>
